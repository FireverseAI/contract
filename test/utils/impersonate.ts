...
export const impersonate = async (address: string, provider: JsonRpcProvider) => {
...
export const getWalletWithEther = async () => {
  ...
  wallet = new ethers.Wallet(wallet.privateKey, ethers.provider)
  await faucet(wallet.address, ethers.provider)
  return wallet
}

export const stopImpersonation = async (address: string, provider: JsonRpcProvider) => {
  await provider.send('hardhat_stopImpersonatingAccount', [address])
}

export const whileImpersonating = async <T>(address: string, provider: JsonRpcProvider, fn: () => T) => {
